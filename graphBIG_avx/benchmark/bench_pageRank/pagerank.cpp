...
__attribute__((aligned(64))) float *pr, *old_pr, *result;

__m512 pr_vec, old_pr_vec, d_vec;
__m512 zero_vec = _mm512_setzero_ps();
vertex_iterator vit1, vit2, vit3, vit4, vit5, vit6, vit7, vit8, vit9, vit10, vit11, vit12, vit13, vit14, vit15, vit16;
        ...
        int perf_group)
{
    ...
    /*
    __attribute__((aligned(64))) float *pr, *old_pr, *result;
    pr = (float *)_mm_malloc(16 * sizeof(float), 64);
    old_pr = (float *)_mm_malloc(16 * sizeof(float), 64);
    result = (float *)_mm_malloc(16 * sizeof(float), 64);
    
    assert(pr && old_pr && result);

    memset(pr, 0, 16 * sizeof(float));
    memset(old_pr, 0, 16 * sizeof(float));
    ...
    */
    ...
    #pragma omp parallel num_threads(threadnum)
    {
		...
		cout<<"Count: "<<countt<<"\n";
            ...
            for (unsigned vid=start;vid<end;vid+=16)
            {
                ...
                vit5 = g.find_vertex(vid + 4);
                vit6 = g.find_vertex(vid + 5);
                vit7 = g.find_vertex(vid + 6);
                vit8 = g.find_vertex(vid + 7);

                vit9 = g.find_vertex(vid + 8);
                vit10 = g.find_vertex(vid + 9);
                vit11 = g.find_vertex(vid + 10);
                vit12 = g.find_vertex(vid + 11);

                vit13 = g.find_vertex(vid + 12);
                vit14 = g.find_vertex(vid + 13);
                vit15 = g.find_vertex(vid + 14);
                vit16 = g.find_vertex(vid + 15);
                ...
                pr[4] = vit5->property().pr;   old_pr[4] = vit5->property().old_pr;
                pr[5] = vit6->property().pr;   old_pr[5] = vit6->property().old_pr;
                pr[6] = vit7->property().pr;   old_pr[6] = vit7->property().old_pr;
                pr[7] = vit8->property().pr;   old_pr[7] = vit8->property().old_pr;

                pr[8] = vit9->property().pr;     old_pr[8] = vit9->property().old_pr;
                pr[9] = vit10->property().pr;    old_pr[9] = vit10->property().old_pr;
                pr[10] = vit11->property().pr;   old_pr[10] = vit11->property().old_pr;
                pr[11] = vit12->property().pr;   old_pr[11] = vit12->property().old_pr;

                pr[12] = vit13->property().pr;   old_pr[12] = vit13->property().old_pr;
                pr[13] = vit14->property().pr;   old_pr[13] = vit14->property().old_pr;
                pr[14] = vit15->property().pr;   old_pr[14] = vit15->property().old_pr;
                pr[15] = vit16->property().pr;   old_pr[15] = vit16->property().old_pr;

                pr_vec = _mm512_load_ps(&pr[0]);
                old_pr_vec = _mm512_load_ps(&old_pr[0]);

                d_vec = _mm512_sub_ps(pr_vec, old_pr_vec); // Subtract pr vectors and store in d
                d_vec = _mm512_mul_ps(d_vec, d_vec); // Square the values in d_vec
                //d_vec = _mm_hadd_ps(_mm_hadd_ps(d_vec, zero_vec), zero_vec); // Sum all elements in d_vec
                e_vec_sum += _mm512_reduce_add_ps(d_vec);
