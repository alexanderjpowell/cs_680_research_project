...
void gibbs_sampler(graph_t * g, set<vid_t> & evidence_nodes)
{
    ...
    for (vit_t vit=g->vertices_begin(); vit!=g->vertices_end(); vit++)
    {
        ...
        if (evidence_nodes.find(vid) == evidence_nodes.end())    // not evidence
        {
            ...
            __attribute__((aligned(16))) float *norm_factor_ptr, *prob_ptr;
            norm_factor_ptr = (float *)_mm_malloc(4 * sizeof(float), 16);
            prob_ptr = (float *)_mm_malloc(4 * sizeof(float), 16);

            assert(norm_factor_ptr && prob_ptr);

            memset(norm_factor_ptr, 0, 4 * sizeof(float));
            memset(prob_ptr, 0, 4 * sizeof(float));

            __m128 norm_vec, prob_vec;
            ...
            for (size_t i=0; i<vit->property().num_values; i++)
            {
                prob_ptr[i] = vit->property().prob[i];
                prob_vec = _mm_load_ps(&prob_ptr[0]);

                prob_vec = _mm_hadd_ps(_mm_hadd_ps(prob_vec, zero_vec), zero_vec);
                _mm_store_ss(&prob_ptr[0], prob_vec);

                norm_factor += prob_ptr[0];
