...
size_t triangle_count(graph_t& g, gBenchPerf_event & perf, int perf_group)
{
    ...
    __m128 vertex_count_vec;
    __m128 two_vec = _mm_set_ps1(2.0);
    __m128 zero_vec = _mm_setzero_ps();

    __attribute__((aligned(16))) float *vertex_count, *result;
    vertex_count = (float *)_mm_malloc(4 * sizeof(float), 16);
    result = (float *)_mm_malloc(4 * sizeof(float), 16);
    assert(vertex_count && result);
    memset(vertex_count, 0, 4 * sizeof(float));
    memset(result, 0, 4 * sizeof(float));

    // tune the per-vertex count
    for (vertex_iterator vit=g.vertices_begin(); vit!=g.vertices_end();)
    {
        vertex_count[0] = vit->property().count; vit++;
        vertex_count[1] = vit->property().count; vit++;
        vertex_count[2] = vit->property().count; vit++;
        vertex_count[3] = vit->property().count; vit++;

        vertex_count_vec = _mm_load_ps(&vertex_count[0]);
        vertex_count_vec = _mm_div_ps(vertex_count_vec, two_vec);

        vertex_count_vec = _mm_hadd_ps(_mm_hadd_ps(vertex_count_vec, zero_vec), zero_vec); // Sum all elements in vertex_count_vec

        _mm_store_ss(&result[0], vertex_count_vec);
...
void gen_workset(graph_t& g, vector<unsigned>& workset, unsigned threadnum)
{
    ...
    int count = 0;
    for (vertex_iterator vit=g.vertices_begin(); vit!=g.vertices_end(); vit++, vit++, vit++, vit++)
    {
        count++;
        curr += vit->edges_size();
        if ((curr-last)>=chunk)
